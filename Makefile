...
$(next).xml: $(draft).xml
	...
	sed -e"s/YYYY-MM-DD/$(shell date +%Y-%m-%d)/" ietf-truststore.yang > ietf-truststore\@$(shell date +%Y-%m-%d).yang
	sed -e"s/YYYY-MM-DD/$(shell date +%Y-%m-%d)/" ex-truststore-usage.yang > ex-truststore-usage\@$(shell date +%Y-%m-%d).yang
